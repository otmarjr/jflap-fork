...
public class Discretizer {
    ...
    public static List<CompactCharSequence> orCompact(CompactCharSequence expression) {
        List<CompactCharSequence> se = new LinkedList<>(); // Subexpressions.
        int start = 0;
        int level = 0;
        for (int i = 0; i < expression.length(); i++) {
            if (expression.charAt(i) == '(') {
            ...
            // First level or!
            se.add(delambdaCompact(expression.compactSubSequence(start, i)));
            start = i + 1;
        }
        se.add(delambdaCompact(expression.compactSubSequence(start)));
        return se;
    }
    ...
    public static List<CompactCharSequence> catCompact(CompactCharSequence expression) {
        List<CompactCharSequence> se = new LinkedList<>(); // Subexpressions.
        int start = 0;
        int level = 0;
        for (int i = 0; i < expression.length(); i++) {
    ...
    public static CompactCharSequence delambdaCompact(CompactCharSequence compactString) {
        return compactString.equals(Universe.curProfile.getCompactEmptyString()) ? new CompactCharSequence("") : compactString;
    }
}
